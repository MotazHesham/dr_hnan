    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.joinings.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('qualification') ? 'is-invalid' : '' }}" name="qualification" id="qualification" required>
                    ...
                    @foreach(App\Models\Joining::QUALIFICATION_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('qualification', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('experience_years') ? 'is-invalid' : '' }}" type="number" name="experience_years" id="experience_years" value="{{ old('experience_years', '') }}" step="1" required>
