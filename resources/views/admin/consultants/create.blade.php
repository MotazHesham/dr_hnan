    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.consultants.store') }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label class="required"
                        for="priorty">{{ trans('cruds.consultant.fields.priorty') }}</label>
                    <input class="form-control {{ $errors->has('priorty') ? 'is-invalid' : '' }}" type="number"
                        name="priorty" id="priorty" value="{{ old('priorty', '') }}"
                        required>
