    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.articles.update', [$article->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label class="required" for="writer">{{ trans('cruds.article.fields.writer') }}</label>
                    <input class="form-control {{ $errors->has('writer') ? 'is-invalid' : '' }}" type="text"
                        name="writer" id="writer" value="{{ old('writer', $article->writer) }}" required>
                    @if ($errors->has('writer'))
                        <div class="invalid-feedback">
                            {{ $errors->first('writer') }}
                        </div>
                    @endif
                    <span class="help-block">{{ trans('cruds.article.fields.writer_helper') }}</span>
                </div>
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text"
                        name="date" id="date" value="{{ old('date', $article->date) }}" required>
                    @if ($errors->has('date'))
                        <div class="invalid-feedback">
                            {{ $errors->first('date') }}
                        </div>
                ...
                <div class="form-group">
                    <label class="required" for="file">{{ trans('cruds.article.fields.file') }}</label>
                ...
                <div class="form-group">
                    <div class="form-check {{ $errors->has('active') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="checkbox" name="active" id="active" value="1"
                            {{ $article->active || old('active', 0) === 1 ? 'checked' : '' }} required>
                        <label class="required form-check-label"
                            for="active">{{ trans('cruds.article.fields.active') }}</label>
                    </div>
        ...
        Dropzone.options.fileDropzone = {
            ...
            init: function() {
                @if (isset($article) && $article->file)
                    var file = {!! json_encode($article->file) !!}
