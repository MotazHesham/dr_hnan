@if ($stage_cost_2_pending_color == 'info')
    @if ($user_type == 'client')
        <div class="row">
            ...
            @endif
            <form class="col" method="POST" action="{{ route('client.request-services.update_cost_2') }}" enctype="multipart/form-data">
                <input type="hidden" name="id" value="{{$requestService->id}}" id="">
                @csrf
                <div class="form-group">
                    <label for="cost_2_file" class="required">
                        @if($requestService->cost_2_file)   
                            <a class="btn btn-warning " href="{{ $requestService->cost_2_file->getUrl() }}" target="_blank">
